...
const CoSuperSubmissionStatus = ({ groupId }) => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            // console.log(requests.data.requests[1]);
            const coSupervisorID = requests.data.requests[1].supervisor.supervisorId;

            const coSupervisorRequest = await axios.get(`${BASE_URL}/requests/getRequestsByUserId/${coSupervisorID}`);
            if(coSupervisorRequest.status === 200){
              setRequests(coSupervisorRequest.data.requests);
              console.log("Supervisor request only : ",coSupervisorRequest);
            }
    ...
    return (
        <Container>
            <Grid container md={12} className='status-cards-layout' sx={{ display: 'flex', justifyContent: 'center', }}>
                <Grid item sx={{ mt: 5 }}>
                    <Typography className='title' variant='h5'><span style={{ color: 'black' }}>Co-Supervisor Request Status</span> : {groupId}</Typography>
                    <hr />
                </Grid>
                <Grid item>
...
export default CoSuperSubmissionStatus